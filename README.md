...
- Avoid `maxResults: 1`. One search page is fetched either way, so a single-result run pays a full page of proxy transfer for one row. Values of 25 or more amortize that fetch across many properties.
...
| `maxResults` | integer | no | `25` | Maximum properties per destination, up to 500. The default is one full results page and the best cost per property. |
